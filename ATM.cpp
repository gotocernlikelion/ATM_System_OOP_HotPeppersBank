...
public:
    ...
    void withdraw(Account* account, Bank* userBank, int language_signal) {
        ...
        while (withdrawalsThisSession < maxWithdrawalsPerSession) {
            ...
            if (language_signal == 1) {
                ...
                addTransaction(-1, account->getcardNumber(), "Withdraw", withdrawalAmount, "");
                ...
                addTransaction(Transaction::transaction_counter++, account->getcardNumber(), "Withdraw", withdrawalAmount, ""); //11.18 21:49
                addTransaction(0, account->getcardNumber(), "Withdraw", withdrawalAmount, "");
            }
    ...
    void transfer(Account* sourceAccount, Bank* sourceBank, const vector<Bank*>& allBanks, int language_signal) {
        ...
        if (transferType == 1) { // Cash transfer
            ...
            if (language_signal == 1) {
                ...
                addTransaction(-1, sourceAccount->getcardNumber(), "Transfer", transferAmount, "To: " + destinationAccountNumber);
            }
            else {
                cout << transferAmount << " 원이 성공적으로 송금되었습니다. \n";
                addTransaction(0, sourceAccount->getcardNumber(), "Transfer", transferAmount, "To: " + destinationAccountNumber);
            }
            ...
            if (!session_transactions.empty()) {
                if (language_signal == 1) {
                    ...
                    for (const auto& transaction : session_transactions) {
                        cout << "Card: " << transaction.cardNumber
                    ...
                    cout << "\n거래 요약:\n";
                    for (const auto& transaction : session_transactions) {
                        cout << "카드 번호: " << transaction.cardNumber
                            << ", 거래 타입: " << transaction.transactionType
                            << ", 거래량: " << transaction.amount;
                        if (!transaction.additionalInfo.empty()) {
                            cout << ", 정보: " << transaction.additionalInfo;
                    }
                    cout << endl;
                    }
                }
    ...
    void addTransaction(int id, const string& card, const string& type, int amt, const string& info = "") {
        if(id==0){
            if(type=="Deposit"){
                session_transactions.push_back(Transaction(0,card, "예금", amt, info ));
            }
            else if(type=="Withdraw"){
                session_transactions.push_back(Transaction(0,card, "출금", amt, info ));
            }
            else if(type=="Transfer"){
                session_transactions.push_back(Transaction(0,card, "계좌 이체", amt, info ));
            }
        }
        else if(id==-1){
            session_transactions.push_back(Transaction(0, card, type, amt, info ));
        }
        else{
